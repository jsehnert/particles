...
Usage:
    python apps/noise_collapse_sweep.py --vols 0 5 10
    python apps/noise_collapse_sweep.py --vols 0 5 10 --k-high 3.0 3.5 4.0 4.2 4.5 5.0 \
...
# repo root for cfg_data / extract_candidates_3d, and this script's own dir (apps/) for
# vol_analysis — so it runs both as `python apps/noise_collapse_sweep.py` and as
# `python -m apps.noise_collapse_sweep`.
...
def _resolve_exp_index(
    cfg_path: Path, slab_thickness: int, baseline_method: str
) -> int:
    ...
    for i, e in enumerate(cfg["experiment"]):
        if (
            int(e["slab_thickness"]) == slab_thickness
            and str(e["baseline_method"]) == baseline_method
        ):
            return i
    raise SystemExit(
...
def main() -> None:
    ...
    ap.add_argument(
        "--vols", type=int, nargs="+", required=True, help="cell indices to sweep"
    )
    ap.add_argument(
        "--slab-thickness",
        type=int,
    ...
    ap.add_argument(
        "--k-high", type=float, nargs="+", default=[3.0, 3.5, 4.0, 4.2, 4.5, 5.0]
    )
    ap.add_argument("--m", type=int, nargs="+", default=[1, 2, 3, 4, 5])
    ap.add_argument(
        "--chunk", type=int, default=128, help="z-slices per processing chunk"
    )
    ap.add_argument(
        "--tmp-dir", type=Path, default=PROJECT_ROOT / "Data" / "_tmp_noise_sweep"
    )
    ...
    try:
        for vi in args.vols:
            t0 = time.time()
            print(
                f"\n=== volume index {vi} (exp {exp_index}): precomputing residual ...",
                flush=True,
            )
            src, name, slab_thickness, baseline_method = _build_source(
                vi, exp_index, args.tmp_dir
            )
            counts, seed1, n_support = _sweep_volume(
                src, k_highs, ms, args.chunk, verbose=True
            )
            print(
